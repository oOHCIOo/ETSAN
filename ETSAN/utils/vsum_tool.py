...
def generate_summary(ypred, cps, n_frames, nfps, positions, proportion=0.6, method='knapsack'):
    """
    ...
    Args:
        ...
        Notes: it chooses shots rather than frame-level. Therefore, we need to change.

    """

    n_segs = cps.shape[0] # divide frames into segments.
    ...
    # frame_score shape is (seq_len,)
    # Segment Score
    seg_score = []
    # segment score is the mean of the current segment.
...
def generate_adaptive_summary(probs, summary_rate=0.6):
    # prob_mean = np.mean(probs)
    # valid_ind = []
    # for i in range(len(probs)):
    #     if probs[i] >= prob_mean:
    #         valid_ind.append(i)
    probs_sorted = np.argsort(probs)[::-1]
    valid_ind = probs_sorted[:int(len(probs) * summary_rate)]
    return valid_ind
